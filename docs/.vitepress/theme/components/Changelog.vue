...
const names = computed(() => [props.doc, ...info.value?.alias || []])
const commits = computed(() => {
  const related = allCommits
    .filter(c => c.version || c.docs?.some(i => names.value.includes(i)))
    ...
    <template v-for="(commit, idx) of commits" :key="commit.hash">
      ...
      <template v-else>
        ...
        <div>
          ...
          <span text="sm">
            -
            <span v-html="renderCommitMessage(commit.message.replace(`(${doc})`, ''))" />
          </span>
        </div>
      </template>
    </template>
