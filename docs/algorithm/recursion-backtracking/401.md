...
function readBinaryWatch(num) {
  const res = new Set()
  const vis = Array.from({ length: 10 }).fill(0)
  const check = (hour, minutes) => {
  ...
  const dfs = (t, vis) => {
    if (t == 0) {
      const hour = vis[0] * 1 + vis[1] * 2 + vis[2] * 4 + vis[3] * 8
      const minutes = vis[4] * 1 + vis[5] * 2 + vis[6] * 4 + vis[7] * 8 + vis[8] * 16 + vis[9] * 32
      if (check(hour, minutes)) {
        const tmp = `${hour}:${minutes >= 10 ? minutes : `0${minutes}`}`
        res.add(tmp)
      }
    }
    for (let i = 0; i < 10; i++) {
